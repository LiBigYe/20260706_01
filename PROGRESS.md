...
> 最后更新: 2026-07-19
> 阶段: v5.1 多窗口 Goertzel + 固定 SNR/频谱占优 + 软判决 Chase + PGA112 冻结式 AGC
...
- 接收 DSP 增加 1.1-2.8kHz 浮点带通；首样本按实际 ADC 偏置预置高通状态，差分唤醒、同步、Goertzel 和软判决均使用滤波后的浮点样本。
- 数据段固定使用 SNR=2.0 门限，不再由前导局部驻波抬高整帧门限；目标频点能量达到次强候选的 1.35 倍时，可容忍失真抬高宽带总能量造成的 SNR 低估。
- 数据段连续擦除只保留诊断计数，不再在第 4 个擦除时提前退出，完整符号流交由交织软解码和 CRC 判定。
- PGA112 在进入数据段后锁定写入，SPI 两字节传输期间屏蔽 DMA 回调，避免数据 Goertzel 窗内发生增益阶跃。
- AGC 仅在前导已观测到一次 1500/2400 交替后才允许升档；假前导连续 3 个非导频窗口即退回监听。获取期最高 32x、已验证前导最高 64x，目标范围收窄至 0.5-1.45Vpp，避免微弱误触发把模拟链路推入削顶。
- 前导期档位变化也同步 DSP 噪声底，防止失败前导回到监听后继续使用旧增益标尺。

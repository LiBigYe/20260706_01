...
- 差分能量仅作为前导唤醒门控，降低固定裕量与最小门限；双重导频 Goertzel 锁、SNR 判决和 FEC 仍负责拒绝噪声。
- RX 启动标定阶段检测到有效能量时立即保留该块，防止刚切到接收模式的首帧被写进噪声底。
- 数据末尾加入一个完整 30ms DC 隔离槽，避免时钟偏差使 Postamble 2400Hz 污染 CRC 尾符号；载波绝对能量门限降至 10000，固定增益下由 SNR 锁承担主要判决责任。

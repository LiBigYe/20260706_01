...
uint8_t PGA112_AGC_Update(const uint16_t *samples, uint16_t len, uint8_t frame_active)
{
    ...
    uint16_t hold_thresh = frame_active ? AGC_HOLD_BLOCKS_FAST : AGC_HOLD_BLOCKS;
    uint32_t rms2_low_eff = frame_active
        ? (uint32_t)AGC_RMS_FRAME_LOW * AGC_RMS_FRAME_LOW
        : rms2_low;

    if (rms2 >= rms2_high) {
        agc_lo_hold = 0U;
        if (agc_hi_hold < 0xFFFFU) agc_hi_hold++;
        if (agc_hi_hold >= hold_thresh) {
            agc_hi_hold = 0U;
            if (g_pga_gain_live > PGA_GAIN_MIN_CODE) {
                PGA112_SetGain((uint8_t)(g_pga_gain_live - 1U));
                return 1U;
    ...
    if (rms2 <= rms2_low_eff) {
        agc_hi_hold = 0U;
        if (agc_lo_hold < 0xFFFFU) agc_lo_hold++;
        if (agc_lo_hold >= hold_thresh) {
            agc_lo_hold = 0U;
            if (g_pga_gain_live < PGA_GAIN_MAX_CODE) {
                uint8_t steps = frame_active ? 2U : 1U;
                uint8_t target = (uint8_t)(g_pga_gain_live + steps);
                if (target > PGA_GAIN_MAX_CODE)
                    target = PGA_GAIN_MAX_CODE;
                PGA112_SetGain(target);
                return 1U;
            }
        }
        return 0U;

...
{
    /* The classified 160-sample window lies within the sync tone. Search far
     * enough backwards to include the guard-to-sync edge, then take the first
     * near-full 5ms window rather than an arbitrary steady-state maximum. */
...
uint8_t VoiceRx_PushBlock(VoiceRx *rx, const uint16_t *blk)
{
    ...
    case VD_PREAMBLE: {
        ...
        if (d == VP_PILOT_LO || d == VP_PILOT_HI) {
            ...
            /* 自适应 SNR 门限 = clamp(前导均值 × 0.5)。纯净前导的
             * p_noise 会被数值地板钳到 1，未经上限会错误拒绝数据段。 */
